// vim: ts=4:sw=4:et
/* global $ location localStorage alert toastr */
...
function updateNavbar (plugins) {
  ...
  if (plugins === undefined || plugins.enabled === undefined) { return }
...
function printFloat (value, precision) {
  ...
  return isNaN(result) ? '0' : result.toFixed(precision)
}
...
function updateRawValues (rawData) {
  ...
  for (var keyIndex = 0; keyIndex < currencies.length; ++keyIndex) {
    ...
    if ((!isNaN(averageLendingRate) && !isNaN(lentSum)) || !isNaN(totalCoins)) {
      ...
      if (effRateMode === 'lentperc') { effRateModePerc = lentSum / totalCoins }
      ...
      } else { effRateText = makeTooltip('Effective loan rate, considering exchange 15% fee.', 'Eff.') }
...
function Timespan (name, multiplier) {
  ...
  this.formatEarnings = function (currency, earnings, minimizeCurrencyXs) {
    if (currency === 'BTC' && this === Hour) {
      return printFloat(earnings * 100000000, 0) + ' Satoshi / ' + name + '<br/>'
    } else {
      var currencyClass = ''
      if (minimizeCurrencyXs) {
...
function setEffRateMode () {
  ...
  if (validEffRateModes.indexOf(effRateMode) === -1) {
    console.error(effRateMode + ' is not valid effective rate mode!')
